...
Boundary ChemDrawXMLFormat::CalculateMoleculeBoundary(OBMol* pMol)
{
  ...
  pair<vector<double>::iterator, vector<double>::iterator> pXs = minmax_element(atomXs.begin(), atomXs.end());
  pair<vector<double>::iterator, vector<double>::iterator> pYs = minmax_element(atomYs.begin(), atomYs.end());
...
// Calculate necessary shifts when converting to CDXML,
...
bool ChemDrawXMLFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
{
  ...
  for (patom = mol.BeginAtom(i);patom;patom = mol.NextAtom(i))
  {
    ...
    xmlTextWriterWriteFormatAttribute(writer(), C_COORDS , "%f %f", transformCd.x, transformCd.y);
